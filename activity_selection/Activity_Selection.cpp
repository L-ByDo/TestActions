...
The first line consists of an integer T, the number of test cases.
 For each test case, the first line consists of an integer N, the number of
activities. Then the next N lines contain two integers m and n, the start and
...
bool compare(pair<int, int> a,
             pair<int, int> b) // made compare function for sorting
{
...
int main() {
  ...
  while (testcases--) {
    ...
    sort(arr, arr + num, compare); // Inbuilt Sorting Function taking compare
    ...
    cout << "Person can do only " << count << " activities at a time"
         << endl; // Result
    delete[] arr;
  }
  return 0;
